...
namespace NumberConverter
{
	...
	public sealed partial class BlankPage1 : Page
	{
		...
		private void Button_Click_Clean(object sender, ButtonClickArgs e)
		{
			...
			if (input == Result)
				return;
			if (calculatorController.ClearCommand.CanExecute(input))
				calculatorController.ClearCommand.Execute(input);
		}
		...
		private void Backspace_Click(object sender, ButtonClickArgs e)
		{
			...
			if (input == Result)
				return;
			if (calculatorController.BackSpaceCommand.CanExecute(input))
				calculatorController.BackSpaceCommand.Execute(input);
		...
		private void Button_Click_Dot(object sender, ButtonClickArgs e)
		{
			...
			if (input == Result)
				return;
			if (calculatorController.DotCommand.CanExecute(input))
				calculatorController.DotCommand.Execute(input);
		}
		...
		private void From_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			...
			{
				if (sender != Result)
					buttons.VisibleButtonCount = ((InputField) ((ComboBox) sender).DataContext).InputBase;
				calculatorController.CalculateCommand.Execute();
			}
			catch (Exception ee)
			{
		...
		private void InputText_GotFocus(object sender, RoutedEventArgs e)
		{
			...
			buttons.VisibleButtonCount = ((InputField) LastFocusTextBox.DataContext).InputBase;
		...
		private void From_Holding(object sender, EventArgs e)
		{
			FlyoutBase.ShowAttachedFlyout((FrameworkElement) sender);
			parentFlyout = (ComboBox) sender;
		}
		...
		{
			var data = ((InputField) LastFocusTextBox.DataContext);
			SharePages.InputText_KeyUp(sender, e, data.InputBase);
		...
		//private void From_Holding(object sender, RightTappedRoutedEventArgs e)
		//{
