	...
	public class LongDouble
	{
		...
		public string Integer
		{
			...
			set
			{
				if (value.Length > 0 && value[0] == '-')
				{
					...
					IsMinus = false;
				value = new string(value.ToCharArray().SkipWhile((char a) => { return a == '0'; }).ToArray<char>());
				if (value == "")
				{
					integer = "0";
					return;
		...
		public string Fraction
		{
			...
			set
			{
				...
				value = new string(value.ToCharArray().Reverse().SkipWhile((char a) => { return a == '0'; }).Reverse().ToArray<char>()); ;
				if (value == "")
					fraction = "0";
				else
					fraction = value;
